...
# Image paste support — reads clipboard images on Ctrl+V / paste, stores them
# as base64 data-URLs in a hidden input so Streamlit can pick them up on next
# rerun.  The hidden field lives in the parent document so it survives Streamlit
# reruns (which replace the iframe content).
_js_image_paste = (
    "!function(){"
    "if(window.parent.__imgPaste)return;window.parent.__imgPaste=1;"
    "var P=window.parent,D=P.document;"
    # hidden store — survives iframe reruns
    "var box=D.getElementById('__ga_images');"
    "if(!box){box=D.createElement('input');box.type='hidden';"
    "box.id='__ga_images';box.name='__ga_images';D.body.appendChild(box);}"
    "function readImgs(cb){"
    "var cb=P.navigator.clipboard;if(!cb||!cb.read){cb=null;}"
    "var fn=cb?cb.read.bind(cb):null;"
    "if(!fn){cb=null;}"
    "if(!fn){cb=null;}"
    "if(!fn)return cb;"
    "fn().then(function(items){"
    "var out=[];items.forEach(function(item){"
    "item.types.forEach(function(t){"
    "if(t.indexOf('image')===0){"
    "item.getType(t).then(function(blob){"
    "var r=new FileReader();r.onload=function(){out.push(r.result);"
    "if(out.length===items.length||items.length===1)cb(out);};"
    "r.readAsDataURL(blob);}})})});"
    "}"
    # Listen for paste events globally
    "D.addEventListener('paste',function(e){"
    "var items=(e.clipboardData||P.clipboardData).items;"
    "var found=false;"
    "for(var i=0;i<items.length;i++){"
    "if(items[i].type.indexOf('image')===0){"
    "found=true;var blob=items[i].getAsFile();"
...
# Combine all JS
_embed_html(f'<script>{_js_scroll_fix};{_js_ime_fix};{_js_image_paste}</script>', height=0)

# Read any pasted images from the hidden field (set by JS paste handler)
_pasted_images: list[str] = []
if hasattr(st, 'query_params'):
    _img_data = st.query_params.get('__ga_images', '')
else:
    _img_data = ''
# Alternative: check session_state for image data passed via component
if st.session_state.get('_pasted_image_data'):
    _pasted_images = [st.session_state['_pasted_image_data']]
    st.session_state['_pasted_image_data'] = ''
