...
# ---------------------------------------------------------------------------
# Resource monitoring bridge — delegates to resource_monitor module
# ---------------------------------------------------------------------------

def start_resource_monitoring(interval: float = 5.0, max_history: int = 100) -> bool:
    """Start the system resource monitor (CPU/mem/disk).  Returns True if
    started successfully, False if psutil is missing or already running."""
    try:
        from frontends import resource_monitor
        return resource_monitor.start_monitoring(interval, max_history)
    except ImportError:
        return False
...
def get_resource_stats() -> dict | None:
    """Return current system resource snapshot as a dict, or None."""
    try:
        from frontends import resource_monitor
        return resource_monitor.get_current().to_dict()
    except ImportError:
        return None


def get_resource_history(count: int | None = None) -> list[dict]:
    """Return recent resource history as a list of dicts."""
    try:
        from frontends import resource_monitor
        return [s.to_dict() for s in resource_monitor.get_history(count)]
    except ImportError:
        return []


def get_resource_dashboard_json() -> str:
    """Return full resource dashboard data as a JSON string."""
    try:
        from frontends import resource_monitor
        return resource_monitor.get_dashboard_json()
    except ImportError:
        return json.dumps({"error": "psutil not installed", "monitoring": False,
                           "psutil_available": False})


def combined_dashboard_json(thread_name: str | None = None) -> str:
    """Single-call endpoint combining token stats + resource monitoring data
    ...
    if thread_name:
        ...
        result["tokens"] = {
            ...
            "input": ts.input,
            "output": ts.output,
            "cache_create": ts.cache_create,
            "cache_read": ts.cache_read,
    ...
    try:
        from frontends import resource_monitor
        result["resource"] = {
            "current": resource_monitor.get_current().to_dict(),
            "history_count": len(resource_monitor.get_history()),
            "monitoring": resource_monitor.is_monitoring(),
        }
    except ImportError:
        result["resource"] = {"monitoring": False, "psutil_available": False}
