...
API:
  ...
  extract_todos_from_text(text)   → [str, …]  — intent-based todo extraction
...
Intent recognition (auto todo extraction):
  INTENT_PATTERNS — regex list for Chinese task intent detection
  extract_todos_from_text(text) — scan conversation text for actionable tasks
"""
...
# ---------------------------------------------------------------------------
# Intent recognition — auto-extract actionable todos from conversation text
# ---------------------------------------------------------------------------
INTENT_PATTERNS = [
    r'帮我(.{2,40})',
    r'请(?:帮我)?(.{2,40})',
...
_INTENT_RE_LIST = [re.compile(p) for p in INTENT_PATTERNS]
# Filter out noise — very short or very generic matches
_NOISE_RE = re.compile(r'^[。！？\s]{2,}$')


def extract_todos_from_text(text: str) -> list[str]:
    """Scan conversation text for task intents and return deduplicated todo strings.

    Uses INTENT_PATTERNS regex list to find actionable phrases like
    '帮我写个报告', '请帮我检查代码', '需要部署到服务器', etc.
    Results are deduplicated by normalized content.
    """
    if not text:
        return []
    seen: set[str] = set()
    todos: list[str] = []
    for pat in _INTENT_RE_LIST:
        for m in pat.finditer(text):
            # The captured group is the task action/object
            raw = m.group(1).strip().rstrip('。！？,.，')
            if _NOISE_RE.match(raw) or len(raw) < 2:
                continue
            # Normalize for dedup
            key = raw.lower().strip()
            if key not in seen:
